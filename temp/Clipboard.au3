...
if Not IsDeclared("DEBUG") Then Local $DEBUG = True ;show debug messages. To console
...
Func _StopAsClipViewer($hWndA)
	...
	If ($bInChain) Then
		If ($hWndA <> 0) Then
			...
			Local $ret = DllCall("user32.dll", "int", "ChangeClipboardChain", "hwnd", $hWndA, "hwnd", $hWnd_NEXT_IN_CLIP_CHAIN)
		...
		EndIf
		$bInChain = False
		If Not ($LOCAL_ERR) Then Return $ret
	EndIf
	Return 0
EndFunc   ;==>_StopAsClipViewe
